...
- `await rlm.quant.run_backtest(spec_or_prompt, data=None)` — runs an in-memory backtest through the `primequant` engine and returns **only** a compact JSON summary card (`sharpe_ratio`, `sortino_ratio`, `calmar_ratio`, `max_drawdown_pct`, `profit_factor`, `win_rate`, `expectancy_usd`, `trades_count`) plus a `validation_gate` backed by the real CPCV + walk-forward pipeline (Deflated Sharpe Ratio, PBO, IS/OOS degradation). Raw DataFrames, equity curves, and trade lists stay bound in the kernel scope as `_last_df`, `_last_backtest_df`, `_last_equity_curve`, `_last_trades` for subagent inspection — never printed into context (150-token card budget).
- `await rlm.quant.run_pipeline(spec_or_prompt, data=None, param_space=None, report_path=None)` — runs the full workflow: AST lookahead lint -> baseline backtest -> validation gate -> **conditional** Optuna optimization (only when the gate passes and a `param_space` like `{"fast": [5, 20], "slow": [20, 60]}` is given) -> minimalist HTML tearsheet written to disk via `primequant.report.tearsheet`, returning only `{report_path, file_size_kb}`. A failed AST lint returns a `status: "blocked"` card; a failed gate skips optimization but still exports the FAIL-state tearsheet.
