...
/**
 * Bundled quant-skill instructions paragraph, shared between the RLM prompt
 * and the prompt tests so the two never drift.
 */
export const QUANT_SKILLS_PARAGRAPH =
	"Quant research skills are available as `rlm.quant` when the bundled `quant` skill is installed. `await rlm.quant.idea_to_spec(prompt)` parses a trader prompt into a deterministic strategy spec and surfaces assumptions; `await rlm.quant.run_backtest(spec_or_prompt, data=None)` runs an in-memory backtest and returns ONLY a compact JSON summary card (metrics + real CPCV/walk-forward validation gate with DSR/PBO), never raw frames; `await rlm.quant.validate()` returns the validation-gate card; `await rlm.quant.run_pipeline(spec_or_prompt, data=None, param_space=None, report_path=None)` runs the full pipeline (AST lint -> backtest -> validation gate -> conditional Optuna optimization when the gate passes and a param_space is given -> HTML tearsheet on disk, returning only {report_path, file_size_kb}). Raw DataFrames, equity curves, and trade lists stay bound in the kernel scope as `_last_df`, `_last_backtest_df`, `_last_equity_curve`, `_last_trades` — reference them by name for inspection instead of printing them into context. `await rlm.quant.refine_log_failure(...)` records failure patterns into `rlm.harness` memory so future cycles do not repeat them; `await rlm.quant.recall_failures(kind=None)` is the read counterpart — call it BEFORE generating a new strategy idea to fetch the previously-logged failure patterns (returns a compact `{count, failures, prompt_block}` card; fold `prompt_block` into your next generation prompt so known dead-ends are not repeated, closing the fail-and-refine loop).";
