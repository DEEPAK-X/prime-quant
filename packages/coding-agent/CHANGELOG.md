...
- Added MT5 and CSV setup guides in `prime-quant/docs/` and linked them from `prime-quant/README.md`.
...
- Wired the `quant` skill's validation gate to the real CPCV + walk-forward pipeline (Deflated Sharpe Ratio, PBO, IS/OOS degradation) and added `rlm.quant.run_pipeline`, which runs AST lookahead lint -> backtest -> validation gate -> conditional Optuna optimization (only on a passed gate with a param space) -> HTML tearsheet export, keeping all outputs under the 150-token card budget.
